...
class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    func applicationDidFinishLaunching(_ aNotification: Notification) {
        // Hide the Dock icon (if not already set via Info.plist)
    ...
    @objc func cleanDerivedData() {
        ...
        guard let items = try? fileManager.contentsOfDirectory(atPath: derivedDataPath) else {
            self.showNotification(title: "Info", message: "DerivedData folder not found!")
            return
        }
        ...
        DispatchQueue.global(qos: .userInitiated).async {
            ...
            DispatchQueue.main.async {
                self.progressWindow.orderOut(nil)
                self.showNotification(title: "Clean DerivedData", message: "DerivedData cleaned successfully!")
            }
        }
    }
    ...
    @objc func clearXcodeCaches() {
        ...
        guard let items = try? fileManager.contentsOfDirectory(atPath: xcodeCachesPath) else {
            self.showNotification(title: "Info", message: "Xcode Caches folder not found!")
            return
        }
        ...
        DispatchQueue.global(qos: .userInitiated).async {
            ...
            DispatchQueue.main.async { [self] in
                self.progressWindow.orderOut(nil)
                if failedItems.isEmpty {
                    self.showNotification(title: "Clear Xcode Caches", message: "Xcode Caches cleared successfully!")
                } else {
                    self.showNotification(title: "Clear Xcode Caches", message: "Some items could not be removed: \(failedItems.joined(separator: ", "))")
                }
            }
        }
    }
    ...
    @objc func clearArchives() {
        ...
        guard let items = try? fileManager.contentsOfDirectory(atPath: archivesPath) else {
            self.showNotification(title: "Info", message: "Xcode Archives folder not found!")
            return
        }
        ...
        DispatchQueue.global(qos: .userInitiated).async {
            ...
            DispatchQueue.main.async {
                self.progressWindow.orderOut(nil)
                if failedItems.isEmpty {
                    self.showNotification(title: "Clear Archives", message: "Xcode Archives cleared successfully!")
                } else {
                    self.showNotification(title: "Clear Archives", message: "Some items could not be removed: \(failedItems.joined(separator: ", "))")
                }
            }
        }
    }
    ...
    @objc func clearIOSDeviceSupport() {
        let deviceSupportPath = "/Users/\(NSUserName())/Library/Developer/Xcode/iOS DeviceSupport"
        let fileManager = FileManager.default

        guard let items = try? fileManager.contentsOfDirectory(atPath: deviceSupportPath) else {
            self.showNotification(title: "Info", message: "iOS Device Support folder not found!")
            return
        }
        ...
        DispatchQueue.global(qos: .userInitiated).async {
            for (index, item) in items.enumerated() {
                let fullPath = (deviceSupportPath as NSString).appendingPathComponent(item)
                do {
                    try fileManager.removeItem(atPath: fullPath)
                    DispatchQueue.main.async {
                        self.progressBar.doubleValue = Double(index + 1)
            ...
            DispatchQueue.main.async {
                self.progressWindow.orderOut(nil)
                if failedItems.isEmpty {
                    self.showNotification(title: "Clear iOS Device Support", message: "iOS Device Support cleaned successfully!")
                } else {
                    self.showNotification(title: "Clear iOS Device Support", message: "Some items could not be removed: \(failedItems.joined(separator: ", "))")
                }
            }
        }
    }
    ...
    @objc func clearWatchOSDeviceSupport() {
        ...
        guard let items = try? fileManager.contentsOfDirectory(atPath: watchOSDeviceSupportPath) else {
            self.showNotification(title: "Info", message: "watchOS Device Support folder not found!")
            return
        }
        ...
        DispatchQueue.global(qos: .userInitiated).async {
            ...
            DispatchQueue.main.async {
                self.progressWindow.orderOut(nil)
                if failedItems.isEmpty {
                    self.showNotification(title: "Clear watchOS Device Support", message: "watchOS Device Support cleaned successfully!")
                } else {
                    self.showNotification(title: "Clear watchOS Device Support", message: "Some items could not be removed: \(failedItems.joined(separator: ", "))")
                }
            }
        }
    }
    ...
    @objc func clearTVOSDeviceSupport() {
        ...
        guard let items = try? fileManager.contentsOfDirectory(atPath: tvOSDeviceSupportPath) else {
            self.showNotification(title: "Info", message: "tvOS Device Support folder not found!")
            return
        }
        ...
        DispatchQueue.global(qos: .userInitiated).async {
            ...
            DispatchQueue.main.async {
                self.progressWindow.orderOut(nil)
                if failedItems.isEmpty {
                    self.showNotification(title: "Clear tvOS Device Support", message: "tvOS Device Support cleaned successfully!")
                } else {
                    self.showNotification(title: "Clear tvOS Device Support", message: "Some items could not be removed: \(failedItems.joined(separator: ", "))")
                }
            }
        }
    }
    ...
    @objc func removeOldSimulators() {
        ...
        let output = String(data: data, encoding: .utf8) ?? ""
        if task.terminationStatus == 0 {
            showNotification(title: "Remove Old Simulators", message: "Old simulators removed successfully!")
        } else {
            showNotification(title: "Error", message: "Failed to remove old simulators: \(output)")
        }
    }
    ...
    @objc func clearCaches() {
        let cachesPath = "/Users/\(NSUserName())/Library/Developer/Xcode/Caches"
        let fileManager = FileManager.default
        ...
        DispatchQueue.global(qos: .userInitiated).async {
            ...
            DispatchQueue.main.async {
                self.progressWindow.orderOut(nil)
                if failedItems.isEmpty {
                    self.showNotification(title: "Clear Caches", message: "Xcode caches cleared successfully!")
                } else {
                    self.showNotification(title: "Clear Caches", message: "Some items could not be removed: \(failedItems.joined(separator: ", "))")
                }
            }
        }
    }
    ...
    @objc func clearCocoaPodsCache() {
        ...
        if task.terminationStatus == 0 {
            self.showNotification(title: "Clear CocoaPods Cache", message: "CocoaPods cache cleared successfully!")
        } else {
            self.showNotification(title: "Error", message: "Failed to clear CocoaPods cache: \(output)")
        }
    }
    ...
    @objc func emptyTrash() {
        ...
        if task.terminationStatus == 0 {
            self.showNotification(title: "Empty Trash", message: "Trash emptied successfully!")
        } else {
            self.showNotification(title: "Error", message: "Failed to empty trash")
        }
    }
    ...
    @objc func clearAll() {
        ...
        self.showNotification(title: "Clear All", message: "All Xcode caches, archives, and simulators cleared successfully!")
    }
